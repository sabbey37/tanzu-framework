...
func (c *pinnipedController) SetupWithManager(manager ctrl.Manager) error {
	...
	err := ctrl.
		...
		// Filter based on TKr version (only check v1alpha3 clusters/secrets)
...
func (c *pinnipedController) Reconcile(ctx context.Context, req ctrl.Request) (reconcile.Result, error) {
	...
	if (req == ctrl.Request{}) {
		...
		// TODO: how to get this info in vsphere 8
		if err := c.client.List(ctx, addonSecrets, client.InNamespace("tkg-system"),
			client.MatchingLabels{
				//constants.TKGClusterNameLabel: cluster.Name, how to get mgmt cluster name?
				constants.TKGAddonLabel: constants.PinnipedAddonLabel,
			}); err != nil {
			log.Error(err, "Error getting finding management cluster addon secret")
...
func (c *pinnipedController) reconcileAddonSecret(ctx context.Context, cluster clusterapiv1beta1.Cluster) error {
	...
	pinnipedAddonSecretMutateFn := func() error {
		...
		pinnipedDataValues.IdentityManagementType = "none"
